    ...
    <div className="relative min-h-screen w-full bg-[#181818]">
      <div className="relative mx-auto max-w-6xl px-6 pb-24 pt-32 sm:px-8">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white">외근 통계 요약</h1>
          ...
          {monthFilter === null ? (
            <>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-base font-semibold text-white">월별 외근 추이</h2>
                <span className="text-sm text-gray-500">단위: 외근일(일자 분해)</span>
              </div>
              <div className="flex items-end gap-2 overflow-x-auto pb-2">
                {stats.monthDays.map((value, idx) => (
                  <div key={idx} className="flex w-12 flex-shrink-0 flex-col items-center gap-1">
                    <span className="text-sm text-gray-400">{value > 0 ? value : ""}</span>
                    <div
                      className={`w-full rounded-t ${idx + 1 === stats.busyMonth ? "bg-purple-400" : "bg-purple-500/40"}`}
                      style={{ height: `${Math.round((value / monthMax) * 140)}px` }}
                    />
                    <span className="text-sm text-gray-500">{idx + 1}월</span>
                  </div>
                ))}
              </div>
              <p className="mt-3 text-sm text-gray-500">※ {stats.busyMonth}월이 월별 최다 외근일입니다.</p>
            </>
          ) : (
            <>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-base font-semibold text-white">{monthFilter}월 일별 외근 추이</h2>
                <span className="text-sm text-gray-500">단위: 외근일(일자 분해)</span>
              </div>
              <div className="flex items-end gap-1.5 overflow-x-auto pb-2">
                {dayEntries.map(({ day, value }) => (
                  <div key={day} className="flex w-8 flex-shrink-0 flex-col items-center gap-1">
                    ...
                    <div
                      className="w-full rounded-t bg-purple-500/40"
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            <h2 className="mb-4 text-base font-semibold text-white">부서(팀)별 외근일 비중</h2>
                  ...
                  <div key={dept}>
                    <div className="mb-1 flex items-center justify-between text-base">
                      ...
                      <span className="text-gray-400">
                        {pct}% <span className="text-gray-500">({days.toLocaleString()}일)</span>
                      </span>
                    </div>
                    <div className="h-1.5 overflow-hidden rounded-full bg-white/10">
                      <div className="h-full rounded-full bg-purple-400" style={{ width: `${Math.min(100, pct)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
            )}
            <p className="mt-4 text-sm text-gray-500">정렬: 영업 → 공무 → 홍보 → 관리</p>
          </div>

          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            <h2 className="mb-4 text-base font-semibold text-white">개인별 외근일 · TOP3 지역</h2>
                ...
                <table className="w-full min-w-[420px] text-base">
                  ...
                  <tbody>
                    {stats.personList.map((p) => (
                      <tr key={p.name} className="border-b border-white/5 text-gray-200">
...
const StatCard = ({ label, value, note }: { label: string; value: string; note: string }) => (
  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
    ...
    <div className="mt-1 text-sm text-gray-500">{note}</div>
  </div>
);
