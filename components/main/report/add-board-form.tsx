...
const AddBoardForm = ({ candidates }: { candidates: Candidate[] }) => {
  if (candidates.length === 0) {
    return (
      <div className="mb-6 rounded-2xl border border-[#E7E2D2] bg-white px-4 py-3 text-sm text-[#8A8270]">
  ...
  return (
    <form action={createReportBoard} className="mb-6 flex flex-wrap items-center gap-2 rounded-2xl border border-[#E7E2D2] bg-white p-4">
      <span className="text-base font-medium text-[#4B4739]">게시판 추가</span>
      <select
        name="user_id"
        required
