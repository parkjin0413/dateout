...
const ScheduleForm = ({ mode, action, workDate, schedule }: Props) => {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-[#211D14]">{mode === "create" ? "일정 입력" : "일정 수정"}</h1>
        <p className="mt-1 text-base text-[#6B6455]">날짜 {workDate}</p>
      </div>
      ...
      <form action={formAction} className="max-w-2xl space-y-5 rounded-2xl border border-[#E7E2D2] bg-white p-6">
        ...
        <Field label="기간">
          ...
          <p className="mt-1 text-sm text-[#8A8270]">기본은 당일입니다. 여러 날짜에 걸친 일정이면 켜주세요.</p>

          {tripEnabled && (
            <div className="mt-3 grid grid-cols-2 gap-3">
              <div>
                <div className="mb-1 text-sm text-[#6B6455]">시작일</div>
                <input
                  type="date"
                  value={tripStart}
                  onChange={(e) => {
              ...
              <div>
                <div className="mb-1 text-sm text-[#6B6455]">종료일</div>
                <input
                  type="date"
                  value={tripEnd}
                  min={tripStart}
